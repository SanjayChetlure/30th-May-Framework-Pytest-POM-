...
class Test_SwagLabLogin:           #MainClass
    ...
    def test_TC3_LoginToApp_withInValidDetails(self,setup,request):        #test case / test method
        driver=setup
        loginObj=login.SwagLabLoginPage(driver)
        loginObj.enterUN(ReadConfig.getINIFileData("Invalid App Credentials","InvalidUN"))
        self.logger.info("== Invalid UN Entered==")
    ...
    @pytest.mark.smoke
    @pytest.mark.login
    def test_TC4_LoginToApp_withInValidDetails(self, setup, request):  # test case / test method
        driver = setup
        loginObj = login.SwagLabLoginPage(driver)
        loginObj.enterUN(ReadConfig.getINIFileData("Invalid App Credentials", "InvalidUN"))
        self.logger.info("== Invalid UN Entered==")
